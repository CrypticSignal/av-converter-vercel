  ...
  switch (codec) {
    case "AAC":
      return createConversionData(
        `${isKeepVideo ? "-c:v copy " : ""}-c:a aac -b:a ${bitrateSliderValue}k`,
    ...
    case "DTS":
      return createConversionData(
        `${isKeepVideo ? "-c:v copy " : ""}-c:a dca -b:a ${bitrateSliderValue}k -strict -2`,
    ...
    case "MP3":
      ...
      // VBR
      return createConversionData(
        `${isKeepVideo ? "-c:v copy " : ""}-c:a libmp3lame -q:a ${mp3VbrSetting}`,
    ...
    case "Vorbis":
      return createConversionData(
        `-map 0:a -c:a libvorbis ${
          vorbisEncodingType === "abr" ? `-b:a ${bitrateSliderValue}k` : `-q:a ${qValue}`
        }`,
        `${outputName}.ogg`
      );
